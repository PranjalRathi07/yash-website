  ...
  if (isUnauthorized) {
    return (
      <div className="font-sans text-on-surface min-h-screen flex flex-col bg-surface antialiased">
        <main className="grow w-full px-4 md:px-8 lg:px-16 xl:px-24 pt-20 pb-20 flex flex-col items-center justify-center text-center">
          ...
          <p className="font-sans text-base text-on-surface-variant max-w-md mb-8 leading-relaxed">
            Please sign in to view, edit, or checkout the divine garments
            selected for your deity.
          </p>
  ...
  if (isError) {
    return (
      <div className="font-sans text-on-surface min-h-screen flex flex-col bg-surface antialiased justify-center items-center text-center px-6">
        ...
        <h1 className="font-serif text-3xl mb-2 text-on-surface">
          Failed to Load Cart
        </h1>
      ...
      <div className="font-sans text-on-surface min-h-screen flex flex-col bg-surface antialiased">
        <main className="grow w-full px-4 md:px-8 lg:px-16 xl:px-24 pt-20 pb-20 flex flex-col items-center justify-center text-center">
          ...
          <p className="font-sans text-base text-on-surface-variant max-w-md mb-8">
            ...
            crafted to adorn your beloved deity.
          </p>
          <button
            onClick={() => navigate("/collection")}
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            {cartItems.map((item) => {
              ...
              if (item.variant?.size)
                metaParts.push(`Size: ${item.variant.size}`);
              if (item.variant?.color)
                metaParts.push(`Color: ${item.variant.color}`);
              const metaText =
                metaParts.length > 0
                  ? metaParts.join(" | ")
                  : item.product?.category?.name || "Divine Pieces";
              const stockText =
