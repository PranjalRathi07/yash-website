...
export default function Banners() {
	...
	const fetchBanners = useCallback(async () => {
		try {
			...
			setError("Failed to retrieve visual campaigns from the database sanctuary.");
		} finally {
			setLoading(false);
		}
	}, []);
	...
	const handleToggleActive = async (id, currentActive) => {
		try {
			...
			if (response.data.success) {
				...
				alert("Failed to modify banner status: " + response.data.message);
			}
		} catch (err) {
			console.error("Banner toggle error:", err);
			alert("Error updating visibility: " + (err.response?.data?.message || err.message));
		}
	};
	...
	const handleDeleteBanner = async (id) => {
		if (confirm("Are you sure you want to delete this visual campaign banner?")) {
			try {
				...
				if (response.data.success) {
					alert("Banner campaign deleted successfully.");
					fetchBanners();
				} else {
	...
	const handleUploadBanner = async (e) => {
		...
		try {
			...
			alert("An error occurred during upload: " + (err.response?.data?.message || err.message));
		}
	};
	...
	return (
		<div className='min-h-screen bg-surface font-sans text-on-surface flex'>
			...
			<div className='flex-1 ml-64 flex flex-col min-h-screen bg-surface-container-lowest'>
				{/* TOP BAR: TopNavBar */}
				<Topbar searchQuery={searchQuery} setSearchQuery={setSearchQuery} placeholder="Search banners..." />
				...
				<main className='flex-1 max-w-7xl w-full mx-auto p-8 animate-fadeIn'>
					...
					<div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-12'>
						{[
							{ label: "Active Campaigns", value: banners.filter((b) => b.isActive).length.toString() },
							{ label: "Scheduled Banners", value: banners.filter((b) => !b.isActive).length.toString() },
					...
					{loading ? (
						<div className='flex flex-col items-center justify-center p-20 bg-surface text-center rounded-2xl border border-outline-variant/20 shadow-sm'>
							...
							<p className='text-on-surface-variant font-serif text-lg'>Retrieving visual canvases from backend...</p>
						</div>
					) : error ? (
						...
						<div className='border-2 border-dashed border-outline-variant/50 rounded-2xl flex flex-col items-center justify-center p-12 text-center hover:bg-surface-container transition-all group cursor-pointer h-72' onClick={() => setShowUploadModal(true)}>
							...
							<h4 className='font-serif text-lg font-bold text-primary'>No Campaigns Logged</h4>
							<p className='text-xs text-on-surface-variant/70 mt-1'>Click to enshrine your first visual display campaign</p>
						</div>
					) : (
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
								...
								<div
									...
									<div className='flex-1 flex flex-col justify-between'>
										<div>
											...
											<div className='space-y-4 mb-6'>
												<div className='flex items-center justify-between p-3 bg-surface-container rounded-xl border border-outline-variant/10'>
													<span className='text-xs font-semibold text-primary'>Active visibility</span>
													<label className='relative inline-flex items-center cursor-pointer'>
														<input
															type='checkbox'
															checked={camp.isActive}
															onChange={() => handleToggleActive(camp.id, camp.isActive)}
										...
										<div className='flex gap-2 border-t border-outline-variant/15 pt-4'>
											<button
												onClick={() => alert(`Redirect URL target: ${camp.linkUrl || "None"}`)}
							...
							<div onClick={() => setShowUploadModal(true)} className='border-2 border-dashed border-outline-variant/50 rounded-2xl flex flex-col items-center justify-center p-8 text-center hover:bg-surface-container transition-all group cursor-pointer h-100'>
								<div className='w-16 h-16 rounded-full bg-surface-container flex items-center justify-center mb-4 group-hover:bg-tertiary-fixed transition-colors duration-300'>
									<span className='material-symbols-outlined text-[32px] text-on-surface-variant group-hover:text-primary'>add</span>
								</div>
								<h4 className='font-serif text-lg font-bold text-primary'>New Canvas</h4>
								<p className='text-xs text-on-surface-variant/70 mt-1'>Add a new visual display campaign</p>
							</div>
						</div>
					)}
				</main>
				...
				<div className='fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn'>
					<div className='bg-surface w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl p-6 border border-outline-variant/30'>
						<div className='flex justify-between items-center mb-6'>
							<h3 className='font-serif text-2xl font-bold text-primary'>Upload Display Banner</h3>
							<button onClick={() => setShowUploadModal(false)} className='text-outline-variant hover:text-primary focus:outline-none cursor-pointer'>
						...
						<form onSubmit={handleUploadBanner} className='space-y-4'>
									...
									{bannerPreview ? (
										...
										<div className='text-center py-6 relative w-full cursor-pointer hover:bg-surface-container-high/40 transition-colors'>
											<input 
												...
												required
											/>
											<span className='material-symbols-outlined text-[36px] text-tertiary mb-2'>add_photo_alternate</span>
											<p className='text-xs font-bold text-primary'>Select Wide Aspect Banner Image</p>
											<p className='text-[10px] text-on-surface-variant/60 mt-1'>Recommended: 1920x800 px</p>
										</div>
									)}
								</div>
							</div>
							...
							<div className='flex items-center justify-between border-t border-outline-variant/15 pt-4 mt-6'>
								<label className='flex items-center gap-3 cursor-pointer group relative'>
									...
									<span className='text-xs font-bold text-on-surface-variant group-hover:text-primary transition-colors ml-1'>Publish Immediately</span>
								</label>
